...
class Oscilloscope : public AudioStream {
  public:
    ...
    void SetActivityThreshold(int16_t t) { activityThreshold = t; }

  private:
    audio_block_t *inputQueueArray[1];
    ST7735_t3 *display;
    int16_t buffer[AUDIO_BLOCK_SAMPLES];
    int16_t activityThreshold = 100; // default threshold

    // Audio activity detection function
...
void Oscilloscope::update(void) {
  ...
  if (block) {
    ...
    if (bufferBlock == 0) {
      ...
      // No more fillRect: leaves the oscilloscope area untouched when no signal
      // Optionally, you could clear just once upon note-off elsewhere, or draw a message here as desired
    }
  }
}
